...
local function handleRenameCommand()
	-- rename .cs file when renaming class
	...
	vim.ui.input({ prompt = "New Name: " }, function(newName)
		...
		local isCsharpFile = string.sub(filePath, -#".cs") == ".cs"
		if isCsharpFile then
			local pattern = "[%d%a]+%.cs"
			...
			if oldName == fileNameNoExtension and newName ~= fileNameNoExtension then
				...
				for i = 1, #allClients, 1 do
					if allClients[i].name == "omnisharp" then
						lspClient = allClients[i]
					end
				end
				if lspClient == nil then
					print("Omnisharp has not started yet")
					return
				end
				local ms = require("vim.lsp.protocol").Methods
                --local previousRequestId = nil
                --local requestIndex = #lspClient.requests
                --for i=#lspClient.requests,1,-1 do
                --    if(lspClient[i].method == 
                --if previousRequestId ~= nil then
                --    lspClient.cancel_request(previousRequestId)
                --end
	...
	config = function()
		...
		vim.api.nvim_create_autocmd("LspAttach", {
			...
			callback = function(event)
				...
				map("<leader>rn", handleRenameCommand, "[R]e[n]ame")
				...
				if client and client.supports_method(vim.lsp.protocol.Methods.textDocument_documentHighlight) then
					...
					vim.api.nvim_create_autocmd("LspRequest", {
						pattern = { "*.cs" },
						group = vim.api.nvim_create_augroup("csharp-auto-commands", { clear = true }),
						callback = function(event2)
							print(event2.data.request.method .. "    " .. event2.data.request.type)
							--    if
							--		event2.data.request.method == "textDocument/rename"
							--		and event2.data.request.type == "complete"
							--	then
							--		local client2 = vim.lsp.get_client_by_id(event2.data.client_id)
							--		local request = client2.requests[event2.data.request_id]
							--		print("rename completed, buffer: " .. request.bufnr)
							--	end
						end,
					})
				end
